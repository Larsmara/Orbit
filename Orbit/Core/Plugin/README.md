...
- FadeProfiles conditions evaluate via `SecureCmdOptionParse` — the secure game-state axis, not secret values; the catalog is runtime-validated at load. Profiles are priority-ordered (list position, top wins); the highest-priority firing profile's target is the resolved alpha. 0% is legal — the frame is invisible but clickable; Reveal All is the safety net. Edit Mode neutralizes every cap the same way — `Recompute` short-circuits on `Orbit:IsEditMode()` (re-run via EditMode enter/exit callbacks) so no profile dims or mouseover-hides a frame while it is being arranged.
