...
## Description
Standalone, always-on quality-of-life behaviors layered over the Blizzard UI: `MoveMore` (draggable Blizzard panels), `Mouse` (cursor highlight overlay), `Tooltips` (tooltip scale/anchor/hide + background darken + class-color, target, mount, item-level and ID enrichment), `UserInterface` (UI scale below Blizzard's floor + class-color tweaks), `QuestAutomation` (auto accept/turn-in quests), `NpcAutomation` (auto gossip/sell-junk/repair), and `Spotlight` (hotkey universal search, decomposed into `Spotlight/`).
...
Settings land in `Orbit.db.AccountSettings` — runtime modules read the table directly. The config UI is `Core/Config/Advanced/QoL.lua`: an accordion panel (sections: UserInterface, Colors, MoveMore, Mouse, Tooltips, Automation, Spotlight) whose file-local `Get/SetAccountSetting` helpers write the same table. Adding a section = one builder function returning its content height plus one `sectionDefs` row with a localized `PLU_QOL_SEC_*` label. Only one section is expanded at a time (opening one collapses the rest, via a `_onToggle` wrapper). Expanding **Tooltips** shows a live `OrbitTooltipPreview` beside the panel through `Orbit.Tooltips:ShowPreview/RefreshPreview` — a standalone GameTooltip that reuses the real skin path but is **not** in `MANAGED`, so the global tooltip post-calls skip it and its lines are built explicitly from live player data + current settings.
