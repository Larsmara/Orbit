...
| `ObjectivesSkin.lua` | hook-based skinning: headers, blocks, POI, progress/timer bars |
...
- Clipping is toggled, not always on: `OnScrollRangeChanged` enables `SetClipsChildren` only when content overflows (`yRange > 0`) and disables it when everything fits. With no scroll the clip is off, so scenario/event widget button rows and their popouts (e.g. Lorewalking `HelpTip` bubbles anchored past the box edge) render in full instead of clipping. Raising nested widget containers' strata to escape the clip was tried and did **not** work for deep children — gating the clip on real overflow is the reliable fix.
- `ScenarioObjectiveTracker` is header-only, and Orbit skins **no** UIWidget content: widget frames share Blizzard's pool and taint on method call, so the old global `hooksecurefunc` on `UIWidgetTemplate*Mixin:Setup` fired UI-wide and threw secret-value errors on nameplate/area-POI/map widgets (plus a `GetParent` "bad self" crash on forbidden nameplate widgets). Scenario collapse math reads `headerHeight`, so it keeps native header height too.
