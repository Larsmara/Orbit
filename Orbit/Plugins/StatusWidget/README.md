...
- M+ math: with Challenger's Peril (affix 152) the +2/+3 deadlines are `(par-90)*0.8+90` / `(par-90)*0.6+90`; the death penalty is already inside `GetWorldElapsedTime` — adding `timeLost` again double-counts (both bugs shipped once, fixed against WarpDeplete).
